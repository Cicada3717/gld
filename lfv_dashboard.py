    ...
    if not closed.empty:
        ...
        for _, row in df.iterrows():
            if row["action"] in ("BUY", "SELL"):
                ...
                e = None
                for k, v in reversed(list(entry_map.items())):
                    if k[:10] == row["date"]:
                        e = v; break
                pnl_val = row["pnl"]
                ep      = e["price"] if e is not None else ""
                rows.append({
                    "Date":    row["date"],
                    "Entry":   e["time"]  if e is not None else "",
                    "Exit":    row["time"],
                    "Dir":     e["dir"]   if e is not None else row.get("dir", ""),
                    "Units":   int(e["shares"] if e is not None else row.get("shares", 0)),
                    "Entry $": f"${float(ep):,.3f}" if ep != "" else "---",
                    "Exit $":  f"${row['price']:,.3f}",
                    "Stop $":  f"${float(e['stop']):,.3f}" if e is not None else "---",
...
st.caption(f"Liquidity + Fair Value (AVWAP) + Volume Profile  |  "
           f"Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
st.markdown("---")

# ── Toggle ────────────────────────────────────────────────────────────────────
tab_gld, tab_btc = st.tabs(["🥇 GLD — Gold ETF", "₿ BTC-USD — Bitcoin"])
...
with st.sidebar:
    ...
    for key, cfg in ASSETS.items():
        ...
        st.markdown(f"- Trail: BE@2R → ATR trail@3R")
        st.markdown("---")
