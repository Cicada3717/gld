"""
lfv_paper_trader.py - LFV Live Paper Trader.

Runs the BTC-USD LFV strategy in a polling loop and persists state/trades.
The trader replays missed closed bars after restarts so deployments do not
silently skip valid signals that occurred while the process was offline.
"""

import argparse
import csv
import json
import os
import time
from datetime import datetime
from pathlib import Path
from zoneinfo import ZoneInfo

import numpy as np
import pandas as pd
import yfinance as yf

from signal_scanner import detect_signal

ET = ZoneInfo("America/New_York")
UTC = ZoneInfo("UTC")

DATA_DIR = Path(os.environ.get("RAILWAY_VOLUME_MOUNT_PATH", Path(__file__).parent))
YF_CACHE_DIR = DATA_DIR / ".yf_cache"
YF_CACHE_DIR.mkdir(parents=True, exist_ok=True)
yf.set_tz_cache_location(str(YF_CACHE_DIR))

TICKER_CFG = {
    "BTC-USD": {
        "params": {
            "swing_n": 8,
            "sweep_min_atr": 0.40,
            "avwap_tolerance": 0.007,
            "vp_lookback": 80,
            "vp_buckets": 40,
            "vah_val_pct": 0.75,
            "lvn_ratio": 0.20,
            "stop_atr_buffer": 1.0,
            "min_rr": 3.5,
            "be_after_r": 2.0,
            "trail_after_r": 3.0,
            "trail_atr": 0.75,
            "risk_pct": 0.02,
            "leverage": 5.0,
            "commission": 0.0002,
        },
        "fractional_size": True,
        "market_hours": False,
        "eod_close": False,
        "backfill_bars": 576,
    },
}


def _state_file(ticker):
    if ticker == "GLD":
        return DATA_DIR / "lfv_state.json"
    safe = ticker.replace("=", "").replace("-", "")
    return DATA_DIR / f"lfv_state_{safe}.json"


def _trade_file(ticker):
    if ticker == "GLD":
        return DATA_DIR / "lfv_trades.csv"
    safe = ticker.replace("=", "").replace("-", "")
    return DATA_DIR / f"lfv_trades_{safe}.csv"


def load_state(ticker):
    path = _state_file(ticker)
    if path.exists():
        with open(path) as fh:
            return json.load(fh)
    return None


def save_state(ticker, state):
    with open(_state_file(ticker), "w") as fh:
        json.dump(state, fh, indent=2, default=str)


def log_trade(ticker, row):
    path = _trade_file(ticker)
    exists = path.exists()
    with open(path, "a", newline="") as fh:
        writer = csv.DictWriter(
            fh,
            fieldnames=[
                "date",
                "time",
                "action",
                "dir",
                "shares",
                "price",
                "stop",
                "reason",
                "pnl",
                "balance",
                "signal_details",
            ],
        )
        if not exists:
            writer.writeheader()
        writer.writerow(row)


def _bar_time_et(ts):
    ts = pd.Timestamp(ts)
    if ts.tzinfo is None:
        ts = ts.tz_localize(UTC)
    return ts.tz_convert(ET)


def _current_atr(df, period=14):
    high = df["High"].values
    low = df["Low"].values
    close = df["Close"].values
    tr = np.maximum(
        high[1:] - low[1:],
        np.maximum(np.abs(high[1:] - close[:-1]), np.abs(low[1:] - close[:-1])),
    )
    if len(tr) < period:
        return float(np.mean(tr)) if len(tr) > 0 else 1.0
    return float(np.mean(tr[-period:]))


def _qty(balance, price, risk, params):
    if risk <= 0 or price <= 0:
        return 0
    if params.get("fractional_size"):
        risk_shares = balance * params["risk_pct"] / risk
        lev_shares = balance * params["leverage"] / price
        return round(min(risk_shares, lev_shares), 6)
    risk_shares = int(balance * params["risk_pct"] / risk)
    lev_shares = int(balance * params["leverage"] / price)
    return min(risk_shares, lev_shares)


def _close_position(ticker, state, price, reason, params, bar_time=None):
    pos = state["position"]
    if not pos:
        return

    if pos["dir"] == "LONG":
        pnl = (price - pos["entry"]) * pos["shares"]
    else:
        pnl = (pos["entry"] - price) * pos["shares"]
    comm = (pos["entry"] + price) * pos["shares"] * params["commission"]
    net_pnl = pnl - comm

    state["balance"] += net_pnl
    state["total_pnl"] += net_pnl
    state["total_trades"] += 1
    if net_pnl > 0:
        state["wins"] += 1
    else:
        state["losses"] += 1
    state["position"] = None

    now = _bar_time_et(bar_time) if bar_time is not None else datetime.now(ET)
    tag = "WIN" if net_pnl > 0 else "LOSS"
    print(f"\n  [{ticker}] *** CLOSE {pos['dir']} {pos['shares']}sh @ ${price:.3f} ({reason}) [{tag}] ***")
    print(f"      Entry ${pos['entry']:.3f} -> Exit ${price:.3f}  Net: ${net_pnl:+.2f}")
    print(f"      Balance: ${state['balance']:,.2f}")

    log_trade(
        ticker,
        {
            "date": now.strftime("%Y-%m-%d"),
            "time": now.strftime("%H:%M"),
            "action": "CLOSE",
            "dir": pos["dir"],
            "shares": pos["shares"],
            "price": round(price, 3),
            "stop": round(pos["stop"], 3),
            "reason": reason,
            "pnl": round(net_pnl, 2),
            "balance": round(state["balance"], 2),
            "signal_details": (
                f"entry={pos['entry']:.3f} phase={pos.get('phase',1)} "
                f"swept={pos.get('swept_lvl','?')} poc={pos.get('poc','?')}"
            ),
        },
    )
    save_state(ticker, state)


def _manage_position(ticker, state, price, atr, params, bar_time=None):
    if not state["position"]:
        return

    pos = state["position"]
    init_risk = pos["init_risk"]
    entry = pos["entry"]
    phase = pos.get("phase", 1)

    if pos["dir"] == "LONG":
        profit_r = (price - entry) / init_risk if init_risk > 0 else 0

        if phase == 1 and profit_r >= params["be_after_r"]:
            new_stop = entry
            if new_stop > pos["stop"]:
                pos["stop"] = new_stop
                pos["phase"] = 2
                save_state(ticker, state)

        if phase <= 2 and profit_r >= params["trail_after_r"]:
            new_stop = price - atr * params["trail_atr"]
            if new_stop > pos["stop"]:
                pos["stop"] = new_stop
                pos["phase"] = 3
                save_state(ticker, state)

        if pos.get("phase", phase) == 3:
            new_stop = price - atr * params["trail_atr"]
            if new_stop > pos["stop"]:
                pos["stop"] = new_stop
                save_state(ticker, state)

        if price <= pos["stop"]:
            reason = {1: "STOP", 2: "BE_STOP", 3: "TRAIL_STOP"}.get(pos.get("phase", phase), "STOP")
            _close_position(ticker, state, pos["stop"], reason, params, bar_time=bar_time)

    else:
        profit_r = (entry - price) / init_risk if init_risk > 0 else 0

        if phase == 1 and profit_r >= params["be_after_r"]:
            new_stop = entry
            if new_stop < pos["stop"]:
                pos["stop"] = new_stop
                pos["phase"] = 2
                save_state(ticker, state)

        if phase <= 2 and profit_r >= params["trail_after_r"]:
            new_stop = price + atr * params["trail_atr"]
            if new_stop < pos["stop"]:
                pos["stop"] = new_stop
                pos["phase"] = 3
                save_state(ticker, state)

        if pos.get("phase", phase) == 3:
            new_stop = price + atr * params["trail_atr"]
            if new_stop < pos["stop"]:
                pos["stop"] = new_stop
                save_state(ticker, state)

        if price >= pos["stop"]:
            reason = {1: "STOP", 2: "BE_STOP", 3: "TRAIL_STOP"}.get(pos.get("phase", phase), "STOP")
            _close_position(ticker, state, pos["stop"], reason, params, bar_time=bar_time)


def _is_market_open_gld():
    now = datetime.now(ET)
    if now.weekday() >= 5:
        return False
    if now.hour < 9 or (now.hour == 9 and now.minute < 30):
        return False
    if now.hour >= 16:
        return False
    return True


def _should_eod_close_gld():
    now = datetime.now(ET)
    return now.weekday() < 5 and now.hour >= 16 and now.hour < 17


def process_ticker(ticker, state, last_signal_bar, cfg):
    params = {**cfg["params"], "fractional_size": cfg.get("fractional_size", False)}
    now = datetime.now(ET)

    if cfg["market_hours"] and not _is_market_open_gld():
        if cfg["eod_close"] and state["position"] and _should_eod_close_gld():
            print(f"  [{ticker}] EOD -- closing position")
            df = yf.download(ticker, period="1d", interval="5m", auto_adjust=True, progress=False)
            if isinstance(df.columns, pd.MultiIndex):
                df.columns = df.columns.droplevel(1)
            df.dropna(inplace=True)
            if not df.empty:
                _close_position(ticker, state, float(df["Close"].iloc[-1]), "EOD", params)
        return state.get("last_signal_bar", last_signal_bar)

    df = yf.download(ticker, period="60d", interval="5m", auto_adjust=True, progress=False)
    if isinstance(df.columns, pd.MultiIndex):
        df.columns = df.columns.droplevel(1)
    df.dropna(inplace=True)

    if len(df) < 50:
        print(f"  [{ticker}] Insufficient data ({len(df)} bars)")
        return state.get("last_signal_bar", last_signal_bar)

    price = float(df["Close"].iloc[-1])
    atr = _current_atr(df)
    print(f"  [{now.strftime('%H:%M')}] {ticker} ${price:,.3f}  ATR={atr:.3f}  Bars={len(df)}")

    sig_cfg = {"ticker": ticker, **params}
    last_processed_raw = state.get("last_processed_bar")
    if last_processed_raw:
        last_processed = pd.Timestamp(last_processed_raw)
    else:
        backfill_bars = min(len(df) - 1, cfg.get("backfill_bars", 288))
        last_processed = df.index[-backfill_bars - 1]

    processed_any = False
    for current_ts in df.index[df.index > last_processed]:
        sub = df.loc[:current_ts]
        if len(sub) < 50:
            continue

        bar_price = float(sub["Close"].iloc[-1])
        bar_atr = _current_atr(sub)

        if state["position"]:
            _manage_position(ticker, state, bar_price, bar_atr, params, bar_time=current_ts)

        if not state["position"]:
            sig = detect_signal(sub, sig_cfg)
            if sig is not None:
                bar_key = str(sig["bar_time"])
                if state.get("last_signal_bar") != bar_key:
                    entry = sig["entry"]
                    stop = sig["stop"]
                    stop_dist = abs(entry - stop)
                    shares = _qty(state["balance"], entry, stop_dist, params)
                    if shares > 0:
                        direction = sig["direction"]
                        action = "BUY" if direction == "LONG" else "SELL"
                        entry_dt = _bar_time_et(sig["bar_time"])
                        proj_rr = sig.get("proj_rr", 0)

                        state["position"] = {
                            "dir": direction,
                            "shares": shares,
                            "entry": entry,
                            "stop": stop,
                            "init_risk": stop_dist,
                            "phase": 1,
                            "entry_time": entry_dt.strftime("%Y-%m-%d %H:%M"),
                            "swept_lvl": sig.get("swept_lvl"),
                            "poc": sig.get("poc"),
                            "avwap": sig.get("avwap"),
                        }
                        save_state(ticker, state)

                        print(
                            f"\n  [{ticker}] >>> ENTER {direction} {shares}sh @ ${entry:,.3f}  "
                            f"Stop:${stop:,.3f}  ProjRR:{proj_rr:.1f}x  "
                            f"Swept:{sig.get('swept_lvl',0):,.3f}"
                        )

                        log_trade(
                            ticker,
                            {
                                "date": entry_dt.strftime("%Y-%m-%d"),
                                "time": entry_dt.strftime("%H:%M"),
                                "action": action,
                                "dir": direction,
                                "shares": shares,
                                "price": round(entry, 3),
                                "stop": round(stop, 3),
                                "reason": f"LFV sweep @ {sig.get('swept_lvl',0):.3f}",
                                "pnl": "",
                                "balance": round(state["balance"], 2),
                                "signal_details": (
                                    f"swept={sig.get('swept_lvl',0):.3f} "
                                    f"avwap={sig.get('avwap',0):.3f} "
                                    f"poc={sig.get('poc',0):.3f} "
                                    f"proj_rr={proj_rr:.1f}"
                                ),
                            },
                        )

                    state["last_signal_bar"] = bar_key

        state["last_processed_bar"] = str(current_ts)
        processed_any = True

    if processed_any:
        save_state(ticker, state)
    else:
        sig = detect_signal(df, sig_cfg)
        if sig is None:
            print(f"      [{ticker}] No signal")

    if state["position"]:
        pos = state["position"]
        unreal = (
            (price - pos["entry"]) * pos["shares"]
            if pos["dir"] == "LONG"
            else (pos["entry"] - price) * pos["shares"]
        )
        phase_name = {1: "HARD", 2: "BE", 3: "TRAIL"}.get(pos.get("phase", 1), "?")
        print(
            f"      [{ticker}] {pos['dir']} {pos['shares']}sh @ ${pos['entry']:.3f}  "
            f"Stop:${pos['stop']:.3f}[{phase_name}]  Unreal:${unreal:+.2f}"
        )

    return state.get("last_signal_bar", last_signal_bar)


def run(capital=10_000.0):
    tickers = list(TICKER_CFG.keys())

    print(f"\n{'='*70}")
    print(f"  LFV Paper Trader  |  {', '.join(tickers)}  |  5-min bars")
    print(f"  Capital per asset: ${capital:,.0f}")
    print(f"{'='*70}\n")

    states = {}
    last_signal_bars = {}
    for ticker in tickers:
        state = load_state(ticker)
        if state and state.get("ticker") == ticker:
            print(
                f"  [{ticker}] Resuming -- Balance: ${state['balance']:,.2f}, "
                f"Trades: {state['total_trades']}, P&L: ${state['total_pnl']:+,.2f}"
            )
        else:
            state = {
                "ticker": ticker,
                "capital": capital,
                "balance": capital,
                "position": None,
                "total_trades": 0,
                "total_pnl": 0.0,
                "wins": 0,
                "losses": 0,
                "last_signal_bar": None,
                "last_processed_bar": None,
            }
            save_state(ticker, state)
            print(f"  [{ticker}] Starting fresh -- ${capital:,.0f}")
        states[ticker] = state
        last_signal_bars[ticker] = state.get("last_signal_bar")

    while True:
        for ticker in tickers:
            try:
                last_signal_bars[ticker] = process_ticker(
                    ticker,
                    states[ticker],
                    last_signal_bars[ticker],
                    TICKER_CFG[ticker],
                )
            except Exception as exc:
                print(f"  [{ticker}] ERROR: {exc}")
                import traceback

                traceback.print_exc()

        now_ts = time.time()
        next_5m = (now_ts // 300 + 1) * 300 + 10
        time.sleep(next_5m - now_ts)


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--capital", default=10_000.0, type=float, help="Starting capital per asset")
    args = ap.parse_args()
    run(capital=args.capital)


if __name__ == "__main__":
    main()
